...
function convert(value, fromUnit, toUnit) {
  ...
  if (!toFactor)
    throw new Error(
      `Unknown unit: "${toUnit}" (or it belongs to a different category than "${fromUnit}")`,
    );
...
module.exports = {
  ...
  async unit_converter({ value, from_unit, to_unit }) {
    ...
    if (!fromCat)
      throw new Error(
        `Unrecognised unit: "${from_unit}". Supported: length (m/km/ft/in/mi/cm), mass (kg/g/lb/oz), temperature (C/F/K), data (B/KB/MB/GB/TB)`,
      );
    if (!toCat) throw new Error(`Unrecognised unit: "${to_unit}"`);
    if (fromCat !== toCat)
      throw new Error(
        `Cannot convert between different categories: "${from_unit}" (${fromCat}) and "${to_unit}" (${toCat})`,
      );
