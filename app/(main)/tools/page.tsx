...
import {
  Server, HardDrive, Loader2, Play, CheckCircle2, XCircle,
  Wrench, AlertTriangle, FunctionSquare, BookOpen, ArrowLeft,
} from 'lucide-react';
...
export default function ToolsPage() {
  ...
  const currentTools =
    tab === 'builtin'       ? builtins :
    tab === 'skills'        ? skills :
    tab === 'function_tool' ? functionTools : mcp;
    ...
    try {
      const res = await fetch('/api/tools', {
        ...
        body: JSON.stringify({
          ...
          toolType: tab === 'builtin' ? 'builtin' : tab === 'function_tool' ? 'function_tool' : 'mcp',
          args:     parsed,
        }),
      });
      const data = await res.json();
    ...
    <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-amber-200 text-sm">Test built-in tools, function tools, and MCP tools. Preview skill instructions.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-shrink-0 bg-red-50 dark:bg-red-950/60 border-b border-red-200 dark:border-red-800">
        <div className="max-w-6xl mx-auto px-8 py-2.5 flex items-center gap-2.5 text-sm text-red-700 dark:text-red-300">
          ...
          <span>
            Tools execute directly on the server. <strong>Test cautiously</strong> — file writes, deletes, and shell commands can damage the system or cause data loss.
          </span>
        </div>
      </div>
        ...
        <div className="flex-1 flex flex-col min-h-0 overflow-y-auto px-4 md:px-8 py-4 md:py-6">
          ...
          {!settingsLoaded ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-gray-400 dark:text-gray-500">
              ...
              <p className="text-sm font-600 text-gray-500 dark:text-gray-400 mb-1">Tool Playground is disabled</p>
              <p className="text-sm">Enable it in <a href="/settings" className="text-amber-600 dark:text-amber-400 hover:underline">Settings</a> to test tools interactively.</p>
            </div>
          ) : loading ? (
            <div className="flex items-center justify-center gap-2 py-12 text-gray-400">
              <Loader2 size={18} className="animate-spin" />
            ...
            <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded-xl p-4 text-red-700 dark:text-red-300 text-sm">{error}</div>
          ) : allCount === 0 ? (
            <div className="text-center py-12 text-gray-400 dark:text-gray-500">
              <Wrench size={32} className="mx-auto mb-3 opacity-50" />
              <p className="text-sm">No tools available. Configure your API key in Settings first.</p>
            </div>
          ) : (
            <div className="flex flex-col md:flex-row gap-6 flex-1 min-h-0">
              {/* Tool list sidebar — on mobile, hide when a tool is selected */}
              <div className={`w-full md:w-100 flex-shrink-0 flex flex-col ${selectedTool ? 'hidden md:flex' : 'flex'}`}>
                ...
                <div className="flex gap-1 mb-4 bg-gray-100 dark:bg-gray-800 rounded-lg p-1 flex-wrap">
                  {([
                    ...
                    <button
                      key={id}
                      title={title}
                      onClick={() => { setTab(id); setSelectedTool(null); }}
                ...
                {isSkillTab && (
                  <div className="mb-3 px-3 py-2 rounded-lg bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 text-sm text-blue-700 dark:text-blue-300">
                    <strong>Skills are instruction modules</strong>, not callable functions.
                    Their SKILL.md content is injected into the agent system prompt.
                  </div>
                )}
                ...
                {tab === 'function_tool' && (
                  <div className="mb-3 px-3 py-2 rounded-lg bg-purple-50 dark:bg-purple-950/30 border border-purple-200 dark:border-purple-800 text-sm text-purple-700 dark:text-purple-300">
                    <strong>Function tools</strong> follow the OpenAI function-calling spec.
                    The model emits a <code>tool_call</code>; the server runs <code>index.js</code> in a subprocess.
                  </div>
                )}
                ...
                <div className="flex-1 space-y-1 overflow-y-auto pr-1">
                  ...
                  {currentTools.length === 0 && (
                    <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-4">No tools in this category</p>
                  )}
                </div>
              </div>
              ...
              <div className={`flex-1 min-w-0 flex-col min-h-0 overflow-hidden ${selectedTool ? 'flex' : 'hidden md:flex'}`}>
                {selectedTool ? (
                  <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col flex-1 min-h-0 overflow-hidden">
                    ...
                    <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
                      <div className="flex items-center gap-2">
                        ...
                        <h2 className="font-mono text-sm font-700 text-gray-900 dark:text-gray-100">{selectedTool.name}</h2>
                      </div>
                      <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{selectedTool.description}</p>
                      {selectedTool.source !== selectedTool.name && (
                        <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Source: {selectedTool.source}</p>
                      )}
                    </div>
                    ...
                    <div className={`flex-shrink-0 overflow-y-auto ${isSkillTab ? 'max-h-[100%]' : 'h-full'}`}>
                    {isSkillTab ? (
                      executing && !result ? (
                        <div className="px-5 py-4 flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                          <Loader2 size={14} className="animate-spin" /> Loading skill content…
                        </div>
                      ) : null
                    ) : (
                        ...
                        if (keys.length === 0) {
                          return (
                            <div className="px-5 py-4">
                              <p className="text-sm text-gray-400 dark:text-gray-500 mb-3">This tool takes no parameters.</p>
                              <button
                    ...
                    {result !== null && !execError && (
                      ...
                      <div className={
                        ...
                        <div className="flex items-center gap-1.5 mb-1 text-sm font-600 text-emerald-700 dark:text-emerald-300 flex-shrink-0">
                          <CheckCircle2 size={14} /> {isSkillTab ? 'Skill Content (injected into system prompt)' : 'Result'}
                        </div>
                        {/* Skills: render SKILL.md content as preformatted text */}
                        {isSkillTab && typeof result === 'object' && result !== null && 'content' in result ? (
                          ...
                          <JsonView value={result} stringPassthrough initialDepth={2} maxHeight="max-h-64" />
                        )}
                      </div>
                    )}
                    </div>{/* end form/loader region */}
                  ...
                  <div className="flex flex-col items-center justify-center h-64 text-gray-400 dark:text-gray-500">
                    ...
                    <p className="text-sm mt-1 opacity-60">Choose a category tab, then click a tool to test it</p>
                  </div>
                )}
              </div>
            </div>
