          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-blue-200 text-sm">
                Three quick setup steps. Don&apos;t worry, you can change everything later inside
                the app.
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 px-1">
          {steps.map((label, i) => (
            <div key={label} className="flex-1 flex items-center gap-2">
              <div
                className={`h-7 w-7 rounded-full flex items-center justify-center text-sm font-800 ${i <= step ? 'bg-blue-500 text-white' : 'bg-gray-800 text-gray-500'}`}
              >
                {i + 1}
              </div>
              <span
                className={`text-sm font-700 hidden sm:inline ${i <= step ? 'text-gray-100' : 'text-gray-500'}`}
              >
                {label}
              </span>
              {i < steps.length - 1 && (
                <div className={`h-0.5 flex-1 ${i < step ? 'bg-blue-500' : 'bg-gray-800'}`} />
              )}
            </div>
          ))}
        </div>
        ...
        <div className="rounded-2xl bg-gray-800 border border-gray-700 shadow-xl overflow-visible">
          {step === 0 && (
            <div className="p-4 space-y-3">
              <div className="flex items-center gap-2 text-gray-200 font-800">
                <Globe size={16} className="text-blue-400" /> OpenAI-compatible API endpoint and key
              </div>
              <input
                type="url"
                value={endpoint}
                ...
                className="w-full h-10 bg-gray-900 text-gray-100 px-3 rounded-lg border border-gray-600 text-sm focus:outline-none focus:border-blue-500 font-mono"
              />
              <div className="relative">
                <input
                  ...
                  placeholder="API key"
                  className="w-full h-10 bg-gray-900 text-gray-100 pl-3 pr-10 rounded-lg border border-gray-600 text-sm focus:outline-none focus:border-blue-500 font-mono"
                />
                <button
                  type="button"
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 h-10 rounded-lg bg-gray-700 hover:bg-gray-600 disabled:opacity-50 text-sm font-700 text-gray-200"
              >
                {testing ? (
                  <>
                  ...
                  <>
                    <RefreshCw size={14} /> Test & fetch models
                  </>
                )}
              </button>
              {testStatus !== 'idle' && (
                <div
                  className={`flex items-center gap-2 text-sm px-3 py-2 rounded-lg ${testStatus === 'ok' ? 'bg-emerald-950/60 border border-emerald-800 text-emerald-300' : 'bg-red-950/60 border border-red-800 text-red-300'}`}
                >
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-700 text-gray-200">Default model for agents</span>
                </div>
                {models.length > 0 ? (
                  <CustomDropDown
                    models={models}
          ...
          {step === 1 && (
            <div className="p-4 space-y-3">
              <div className="flex items-center gap-2 text-gray-200 font-800">
                <Search size={16} className="text-emerald-400" /> Web search
              </div>
              <label className="flex items-start gap-3 rounded-xl bg-gray-900 border border-gray-700 p-3 cursor-pointer">
                ...
                <span className="text-sm text-gray-300">
                  Enable Exa web search MCP. Get a free key from{' '}
                  <a
                    href="https://exa.ai/"
                    target="_blank"
                    rel="noreferrer"
              ...
              {enableWebSearch && (
                <p className="text-sm text-gray-500">
                  The key is stored only on the Exa MCP server&apos;s environment. You can update it
                  later in Skills &amp; MCP → Exa → Edit → Environment variables.
                </p>
              )}
            </div>
          )}
          ...
          {step === 2 && (
            <div className="p-4 space-y-3">
              <div className="flex items-center gap-2 text-gray-200 font-800">
                <Terminal size={16} className="text-red-400" /> Shell access
              </div>
              <label className="flex items-start gap-3 rounded-xl bg-red-950/30 border border-red-800/60 p-3 cursor-pointer">
                ...
                <span className="text-sm text-red-200">
                  Enable Shell Access to authorize advanced system automation. This allows the agent
                  to run native commands and handle complex workflows natively. Recommended for full
                  agentic capability.
                </span>
              </label>
              {saveError && (
                <div className="text-sm text-red-300 bg-red-950/60 border border-red-800 rounded-lg px-3 py-2">
                  {saveError}
                </div>
              )}
            </div>
          ...
          <div className="p-4 border-t border-gray-700 flex items-center gap-3">
            <button
              type="button"
              onClick={() => (step === 0 ? router.push('/chat') : setStep((s) => s - 1))}
              className="px-4 h-10 rounded-xl border border-gray-600 text-gray-400 hover:text-gray-200 hover:border-gray-500 text-sm font-700 flex items-center gap-2"
            >
              {step === 0 ? (
                'Skip'
              ) : (
            ...
            <button
              type="button"
              onClick={() => (step < 2 ? setStep((s) => s + 1) : saveAll())}
              disabled={saving}
              className="flex-1 flex items-center justify-center gap-2 h-10 rounded-xl bg-blue-600 hover:bg-blue-500 disabled:opacity-60 text-white font-800 text-sm"
            >
              {saving ? (
                <>
