...
export default function Sidebar({
  ...
  const [optimisticSessionId, setOptimisticSessionId] = useState<string | undefined>(
    currentSessionId,
  );
  ...
  useEffect(() => {
    fetch('/api/ui-settings')
      ...
      .then((d: Record<string, unknown>) => {
        if (d.sidebar_workspace_open !== undefined)
          setWorkspaceOpen(d.sidebar_workspace_open !== false);
        if (d.sidebar_system_open !== undefined) setSystemOpen(d.sidebar_system_open !== false);
        if (d.sidebar_pinned_chats_open !== undefined)
          setPinnedChatsOpen(d.sidebar_pinned_chats_open !== false);
        if (d.sidebar_recent_chats_open !== undefined)
          setRecentChatsOpen(d.sidebar_recent_chats_open !== false);
  ...
  const handleContextPinChat = async (id: string, currentlyPinned: boolean) => {
    setContextMenu(null);
    setSessions((prev) =>
      prev.map((s) => (s.id === id ? { ...s, pinned_chat: currentlyPinned ? 0 : 1 } : s)),
    );
    await fetch('/api/sessions', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  const renderSessionItem = (session: Session) => (
    <div
      ...
      onClick={() => {
        if (!editingId) {
          setOptimisticSessionId(session.id);
          onSelectSession(session.id, session.title, session.agent_name);
          onCloseMobile?.();
        }
      }}
      onContextMenu={(e) => {
        ...
        <>
          <button
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
              const pos = getActionMenuPosition(e.currentTarget.getBoundingClientRect());
              openContextMenu(session.id, pos.x, pos.y);
            }}
          ...
          <span className="hidden md:group-hover:flex items-center gap-0.5 flex-shrink-0">
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                handlePinPrompt(session.id, !!session.pinned_prompt);
              }}
    ...
    <aside className="w-full bg-gray-100 dark:bg-gray-900 flex flex-col h-full overflow-hidden border-r border-gray-200 dark:border-gray-800">
      ...
      <div className="px-4 pt-5 pb-4 border-b border-gray-300 dark:border-gray-600 flex-shrink-0">
        <div className="w-full flex items-center gap-1.5 text-left rounded-lg -mx-1 px-1">
          <button
            ...
            className="flex-1 flex items-center gap-2 cursor-pointer rounded-lg transition-colors hover:bg-gray-100 dark:hover:bg-gray-800 py-1"
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-gray-900 dark:text-white font-800 text-base tracking-tight leading-none text-left">
                AgentPrimer
              </p>
              <p className="text-gray-600 dark:text-gray-400 text-sm mt-0.5 truncate text-left">
                AI Agent Platform
              </p>
            </div>
          </button>
      ...
      <div className="px-3 pt-3 pb-1 flex-shrink-0">
        ...
        >
          <Plus
            size={15}
            className={`flex-shrink-0 transition-transform duration-200 ${!isNew ? 'group-hover:rotate-90' : ''}`}
          />
          <span>New Chat</span>
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto min-h-0 px-3 pb-2 space-y-0.5">
        {sessions.length === 0 ? (
          <p className="text-gray-600 dark:text-gray-400 text-sm px-3 py-3 text-center">
          ...
          <>
            {sessions.some((s) => s.pinned_chat) && (
              <>
                <button
                  onClick={() => {
                    setPinnedChatsOpen((o) => {
                      const next = !o;
                      saveUiSetting('sidebar_pinned_chats_open', next);
                      return next;
                    });
                  }}
                ...
                >
                  <ChevronDown
                    size={13}
                    className={`flex-shrink-0 transition-transform duration-200 ${pinnedChatsOpen ? 'rotate-0' : '-rotate-90'}`}
                  />
                  Pinned Chats
                </button>
                <div
                  className={`grid transition-[grid-template-rows] duration-200 ease-in-out ${pinnedChatsOpen ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}
                >
            ...
            {sessions.some((s) => !s.pinned_chat) && (
              <>
                <button
                  onClick={() => {
                    setRecentChatsOpen((o) => {
                      const next = !o;
                      saveUiSetting('sidebar_recent_chats_open', next);
                      return next;
                    });
                  }}
                ...
                >
                  <ChevronDown
                    size={13}
                    className={`flex-shrink-0 transition-transform duration-200 ${recentChatsOpen ? 'rotate-0' : '-rotate-90'}`}
                  />
                  Recent Chats
                </button>
                <div
                  className={`grid transition-[grid-template-rows] duration-200 ease-in-out ${recentChatsOpen ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}
                >
                  <div className="overflow-hidden space-y-0.5">
                    {sessions.filter((s) => !s.pinned_chat).map(renderSessionItem)}
                  </div>
      ...
      <nav className="px-3 py-2 flex-shrink-0">
        <button
          onClick={() => {
            setWorkspaceOpen((o) => {
              const next = !o;
              saveUiSetting('sidebar_workspace_open', next);
              return next;
            });
          }}
        ...
        >
          <ChevronDown
            size={14}
            className={`flex-shrink-0 transition-transform duration-200 ${workspaceOpen ? 'rotate-0' : '-rotate-90'}`}
          />
          Workspace
        </button>
        <div
          className={`grid transition-[grid-template-rows] duration-200 ease-in-out ${workspaceOpen ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}
        >
          <div className="overflow-hidden">{workspaceItems.map(renderNavLink)}</div>
        </div>
      </nav>
      ...
      <nav className="px-3 pb-2 flex-shrink-0">
        <button
          onClick={() => {
            setSystemOpen((o) => {
              const next = !o;
              saveUiSetting('sidebar_system_open', next);
              return next;
            });
          }}
          className="w-full flex items-center gap-1.5 px-2 py-1.5 mb-0.5 rounded text-sm font-600 text-gray-600 dark:text-gray-300 tracking-wider hover:text-gray-900 dark:hover:text-white transition-colors"
        >
          <ChevronDown
            size={14}
            className={`flex-shrink-0 transition-transform duration-200 ${systemOpen ? 'rotate-0' : '-rotate-90'}`}
          />
          System
        </button>
        <div
          className={`grid transition-[grid-template-rows] duration-200 ease-in-out ${systemOpen ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}
        >
          <div className="overflow-hidden">{systemItems.map(renderNavLink)}</div>
        </div>
      </nav>
