...
import {
  CheckCircle2, ChevronRight, Loader2, ShieldAlert, ShieldCheck, Sparkles,
  Wrench, XCircle,
} from 'lucide-react';
...
export function LiveToolCard({
  ...
  return (
    <div className={`w-full min-w-0 rounded-xl border overflow-hidden ${
      ...
      >
        {responseStatus
          ? <CheckCircle2 size={14} className={responseStatus === 'approved' ? 'text-emerald-500' : 'text-gray-400'} />
          : approval
        ...
        <span className="ml-auto flex items-center gap-1.5">
          {isRunning
            ? <><Loader2 size={14} className="animate-spin text-amber-500" /><span className="text-amber-500">Running…</span></>
            : responseStatus === 'approved'
              ? <><CheckCircle2 size={14} className="text-emerald-500" /><span className="text-emerald-600 dark:text-emerald-400">Approved</span></>
              : responseStatus === 'denied'
                ? <><XCircle size={14} className="text-gray-400" /><span className="text-gray-500 dark:text-gray-400">Denied</span></>
                : approval
                  ? <><ShieldAlert size={14} className="text-red-500" /><span className="text-red-600 dark:text-red-400">Approval required</span></>
                  : <><CheckCircle2 size={14} className="text-emerald-500" /><span className="text-emerald-600">Done</span></>
          }
          {!approval && (
            <ChevronRight size={14} className={`text-amber-500 transition-transform duration-200 ${expanded ? 'rotate-90' : ''}`} />
          )}
        </span>
      </button>
      ...
      {approval && !responseStatus && (
        <div className="px-3 pb-3 border-t border-red-200/60 dark:border-red-700/60 space-y-3 mt-0">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            ><ShieldCheck size={14} />Always allow</button>
            <button
              onClick={() => { setResponseStatus('denied'); onApprovalDenied?.(inv); }}
              disabled={approving}
          ...
          <div className="overflow-hidden">
            <div className="px-3 pb-3 border-t border-amber-200/60 dark:border-amber-800/60 space-y-2 mt-0 max-h-72 overflow-y-auto">
              {isRunning ? (
                ...
                <div>
                  <p className="text-amber-700 dark:text-amber-300 font-medium mt-2 mb-1">Arguments</p>
              ...
              {isDone && inv.result !== undefined && (
                <div>
                  ...
                  <div className="bg-white/70 dark:bg-gray-800/70 rounded-lg p-2 max-w-full">
                    <JsonView value={inv.result} stringPassthrough initialDepth={1} maxHeight="max-h-40" />
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="overflow-hidden">
          <div className="px-3 pt-2 pb-3 border-t border-indigo-200/60 dark:border-indigo-800/60 space-y-2">
            <p className="text-sm text-indigo-700/70 dark:text-indigo-300/70 italic leading-snug">
              Only the names and descriptions below are in the system prompt.
              The model loads a skill&rsquo;s full instructions by calling the{' '}
...
export function HistoricalToolsTrace({
  toolCalls, expandByDefault = false,
}: {
  toolCalls: ToolCall[];
  expandByDefault?: boolean;
}) {
  ...
  const fileResults = toolCalls
    .filter(tc => tc.result !== null && typeof tc.result === 'object' && (tc.result as Record<string, unknown>).type === 'agent_file')
    .map(tc => tc.result as AgentFileResult);
  const otherCalls = toolCalls.filter(tc => !(tc.result !== null && typeof tc.result === 'object' && (tc.result as Record<string, unknown>).type === 'agent_file'));

  return (
    <div className="w-full space-y-2">
      {/* Inline file previews */}
      {fileResults.map((f, i) => <AgentFileCard key={i} file={f} />)}
        ...
        <>
          ...
          <div className={`grid transition-[grid-template-rows] duration-200 ease-in-out ${open ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}>
            <div className="overflow-hidden">
              ...
              {otherCalls.map((tc, i) => (
                <div key={i} className="bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-lg p-3 text-sm">
                  <p className="font-700 text-amber-800 dark:text-amber-200 mb-1.5 font-mono">{tc.toolName}</p>
                  ...
                  {tc.result !== undefined && (
                    <>
                      <p className="text-gray-500 dark:text-gray-400 mt-2 mb-1 font-medium">Result:</p>
                      <div className="bg-white dark:bg-gray-800 rounded p-2 max-w-full">
