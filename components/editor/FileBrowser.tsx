  ...
  return nodes.map((n) => {
    if (n.path === targetPath) {
      ...
      const merged = children.map((c) => {
        ...
        return prev ? { ...c, children: prev.children, loaded: prev.loaded } : c;
      });
      return { ...n, children: merged, loaded: true };
    }
    return {
      ...n,
      children: n.children ? setChildren(n.children, targetPath, children) : undefined,
    };
  });
}
...
interface CreateInputProps {
  ...
  setCreating: React.Dispatch<
    React.SetStateAction<{ parentPath: string; type: 'file' | 'dir'; value: string } | null>
  >;
  submitCreate: () => void;
...
export default function FileBrowser({
  ...
  const handleDelete = async (node: TreeNode) => {
    const ok = await showConfirm(`"${node.name}" will be permanently deleted.`, {
  ...
  const submitRename = async () => {
    ...
    const newPath = parentPath(renaming.path) ? `${parentPath(renaming.path)}/${newName}` : newName;
