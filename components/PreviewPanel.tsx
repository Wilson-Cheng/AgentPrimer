...
import { X, RefreshCw, ExternalLink, Save, Check, PanelRight, PanelLeftClose, PanelRightClose, Columns2, ArrowLeft, BookPlus } from 'lucide-react';
...
function detectKind(filePath: string): FileKind {
  ...
  if (['txt', 'json', 'csv', 'xml', 'yaml', 'yml', 'toml', 'js', 'ts', 'css', 'py', 'sh'].includes(ext)) return 'text';
  return 'unknown';
}
...
function AudioPreview({ url, refreshKey, filename }: { url: string; refreshKey: number; filename: string }) {
...
function MarkdownPreview({ filePath, url, refreshKey }: MarkdownPreviewProps) {
  ...
  useEffect(() => {
    ...
    fetch(url)
      .then(r => r.text())
      .then(t => { setSource(t); setLoading(false); setIsDirty(false); })
      .catch(() => { setSource(''); setLoading(false); });
  }, [url, refreshKey]);
    ...
    <div className="flex flex-col h-full">
      ...
      <div ref={splitRef} className="flex flex-1 overflow-hidden relative">
        ...
        {showEditor && (
          ...
          >
            ...
            <textarea
              ref={editorScrollRef}
              value={source}
              onChange={e => { setSource(e.target.value); setIsDirty(true); }}
              spellCheck={false}
              ...
              onScroll={e => {
                ...
                setTimeout(() => { syncingPreview.current = false; }, 50);
              }}
            />
          </div>
        )}
        ...
        {showPreview && (
          <div
            className="flex flex-col overflow-hidden"
            style={paneMode === 'both' ? { width: `${(1 - editorFrac) * 100}%` } : { width: '100%' }}
          >
            {paneMode === 'both' && (
            ...
            <div
              ...
              onScroll={e => {
                ...
                setTimeout(() => { syncingEditor.current = false; }, 50);
              }}
            >
    ...
    <>
      ...
      <div className="flex flex-col w-full overflow-hidden pl-0 md:pl-1">
        ...
        <div className="flex items-center gap-2 pl-18 pr-3 py-4 md:px-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 flex-shrink-0">
          ...
          <span className="flex-1 text-md font-600 text-gray-800 dark:text-gray-200 truncate min-w-0" title={file.path}>
            {title}
          </span>
            ...
            {(kind === 'html' || kind === 'pdf' || kind === 'markdown' || kind === 'text') && (
              <button
                onClick={async () => {
                  ...
                  if (kind === 'pdf') {
                    setRagSnapshot({ kind: 'pdf', title: title.replace(/\.[^.]+$/, ''), pdfUrl: url, filename });
                  } else {
                    // Fetch bytes as text — works for html / md / txt / json etc.
                    try {
                      const res  = await fetch(url);
                      const text = await res.text();
                      const mime = kind === 'html' ? 'text/html'
                        : kind === 'markdown' ? 'text/markdown'
                        : 'text/plain';
                      setRagSnapshot({ kind: 'text', title: title.replace(/\.[^.]+$/, ''), content: text, mime });
                    } catch {
        ...
        <div className="flex-1 overflow-hidden">
          ...
          {kind === 'audio'    && <AudioPreview url={url} refreshKey={refreshKey} filename={title} />}
          {kind === 'pdf'      && <PdfPreview url={url} refreshKey={refreshKey} />}
          {kind === 'markdown' && <MarkdownPreview filePath={file.path} url={url} refreshKey={refreshKey} />}
          {(kind === 'text' || kind === 'unknown') && <TextPreview url={url} refreshKey={refreshKey} />}
        </div>
      </div>
    </div>
