...
describe('ACTIVE_PREVIEW_CONTENT_SECURITY_POLICY', () => {
  ...
  it('can include the concrete preview origin for sandboxed same-domain assets', () => {
    const originDirectives = activePreviewContentSecurityPolicy('http://localhost:15432')
      .split(';')
      .map((d) => d.trim());
    expect(originDirectives).toContain("style-src 'self' http://localhost:15432 'unsafe-inline'");
    expect(originDirectives).toContain(
      "script-src 'self' http://localhost:15432 'unsafe-inline' 'unsafe-eval' blob:",
    );
    expect(originDirectives).toContain("connect-src 'self' http://localhost:15432");
  });

  it('ignores invalid dynamic origins', () => {
    const originDirectives = activePreviewContentSecurityPolicy('not a url')
      .split(';')
      .map((d) => d.trim());
...
describe('isSameOriginPreviewAssetRequest', () => {
  it('allows GET assets referenced by same-origin workspace previews', () => {
    const request = new Request(
      'http://localhost:15432/api/workspace/workspaces/AgentPrimer/data/app/react.production.min.js',
      {
        headers: {
          referer:
            'http://localhost:15432/api/workspace/workspaces/AgentPrimer/data/app/index.html',
        },
      },
    );
  ...
  it('allows GET assets referenced by same-origin editor previews', () => {
    const request = new Request(
      'http://localhost:15432/api/editor/preview/app/react.production.min.js',
      {
  ...
  it('rejects cross-origin referrers and non-preview referrers', () => {
    const crossOrigin = new Request(
      ...
      {
        headers: {
          referer: 'http://example.com/api/workspace/workspaces/AgentPrimer/data/app/index.html',
        },
      },
    );
    const nonPreview = new Request(
      'http://localhost:15432/api/workspace/workspaces/AgentPrimer/data/app/react.production.min.js',
      {
  ...
  it('allows sandboxed iframe style, image, font, and source map assets that omit referrer', () => {
    for (const dest of ['style', 'image', 'font', 'empty']) {
      const request = new Request(
        `http://localhost:15432/api/workspace/workspaces/AgentPrimer/data/projects/react-todo-demo/asset-${dest}`,
        {
  ...
  it('rejects no-referrer preview document requests', () => {
    const request = new Request(
      'http://localhost:15432/api/workspace/workspaces/AgentPrimer/data/projects/react-todo-demo/page2.html',
      {
...
describe('injectPreviewStorageShim', () => {
  it('injects a storage shim into HTML that references localStorage', () => {
    const html =
      '<!doctype html><html><head><title>x</title></head><body><script>localStorage.getItem("x")</script></body></html>';
  ...
  it('does not inject into inert or storage-free content', () => {
    ...
    expect(injectPreviewStorageShim('text/html', '<html><head></head><body></body></html>')).toBe(
