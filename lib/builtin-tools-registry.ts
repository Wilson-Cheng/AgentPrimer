...
export const BUILTIN_TOOLS: BuiltinToolMeta[] = [
  // Output – send files to the user
  {
    id: 'send_file',
    label: 'Send File',
    description: 'Deliver generated or existing files (images, audio, video, PDFs, CSVs…) directly in the chat with inline preview and a download button.',
    category: 'output',
    defaultEnabled: true,
  },
  {
    id: 'open_preview',
    label: 'Open Preview',
    description: 'Open a file in the Preview Panel (HTML apps, images, PDFs, Markdown) so the user can see it without leaving the chat.',
    category: 'output',
    defaultEnabled: true,
  },
  ...
  // Memory
  {
    id: 'append_memory',
    label: 'Append Memory',
    description: "Append notes to this agent's private memory file so it remembers important information across conversations.",
    category: 'memory',
    defaultEnabled: true,
  },
  {
    ...
    description: "Overwrite this agent's private memory file with new content. Dangerous — that agent's existing memory is lost.",
    category: 'memory',
    dangerous: true,
    defaultEnabled: true,
  },
  ...
  // Agent orchestration
  {
    id: 'create_agent',
    label: 'Create Agent',
    description: 'Create a new agent folder with agent.md and memory.md from a name and system prompt.',
    category: 'agent',
    defaultEnabled: true,
  },
  {
    ...
    description: 'Launch a sub-agent as a background task. Returns immediately with a task file to monitor. Sub-agents can nest further sub-agents.',
    category: 'agent',
    defaultEnabled: true,
  },
  {
    ...
    description: 'Append a progress, finished, or error entry to the current async task file. Only available inside async sub-agents.',
    category: 'agent',
    defaultEnabled: true,
  },
  {
    id: 'list_tasks',
    label: 'List Tasks',
    description: 'List async sub-agent tasks where this agent is the assigner or assignee, with current status from the task file.',
    category: 'agent',
    defaultEnabled: true,
  },
  ...
  // Skills — Stage 2 activation (full SKILL.md body on demand)
  {
    id: 'load_skill',
    label: 'Load Skill',
    description: 'Activate one of the available skills by name and load its full SKILL.md body into the conversation. Used for the Stage 2 step of progressive disclosure (Stage 1 names + descriptions are always in the system prompt).',
    category: 'skill',
    defaultEnabled: true,
  },
  ...
  // Filesystem – read
  {
    id: 'read_file',
    label: 'Read Files',
    description: 'Read the contents of any file. Reading hidden/dot files (e.g. .env) requires separate approval.',
    category: 'filesystem',
    defaultEnabled: true,
  },
  {
  ...
  // Filesystem – write
  {
    ...
    description: 'Replace an exact string in an existing file. More token-efficient than write_file — use this for targeted code edits.',
    category: 'filesystem',
    defaultEnabled: true,
  },
  {
  ...
  // Filesystem – destructive
  {
    id: 'delete_path',
    label: 'Delete Files',
    description: 'Permanently delete a file or directory. Each deletion requires user approval unless permanently granted.',
    category: 'filesystem',
    dangerous: true,
    defaultEnabled: true,
  },
  ...
  // RAG
  {
    ...
    label: 'Search RAG',
    description: 'Semantic (vector) or keyword retrieval over indexed documents. Use when the user references uploaded files, asks about stored RAG content, or requests document-grounded answers.',
    category: 'memory',
    defaultEnabled: true,
  },
  ...
  // Shell – most dangerous, opt-in only
  {
    ...
    description: 'Execute arbitrary shell commands on the host system. Grants the agent full root-level access — enable only if you trust the model completely.',
    category: 'shell',
    dangerous: true,
